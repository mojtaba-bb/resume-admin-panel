...
function AboutPartGoal() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "http://localhost:3000/api/page-content/about/goals"
      );
  ...
  const removeItem = (index) => {
    setData((prev) => ({
      ...prev,
      goal: prev.goal.filter((item, i) => i !== index),
    }));
  };
  ...
  const sendData = async ()=>{
    try {
      const res = await axios.post(
        "http://localhost:3000/api/page-content/about/goals",data,
        {
  ...
  return (
      ...
      >
        <div>
          <label className="">goals title</label>
          <br />
        ...
        <div className="">
              ...
              >
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    setChangeIndex(num);
                    setGoal({ en: value.en, fa: value.fa });
                  }}
                >
                  <FaPencilAlt />
                </button>
          ...
          <div className="mt-3 flex gap-3 relative">
            <textarea
              ...
              placeholder="English goals"
              onChange={(e) => {
                e.preventDefault();
                setGoal((prev) => ({ ...prev, en: e.target.value }));
              }}
            ></textarea>
            <textarea
              dir="rtl"
              ...
              placeholder="Persian goals "
              onChange={(e) => {
                e.preventDefault();
                setGoal((prev) => ({ ...prev, fa: e.target.value }));
              }}
            ></textarea>
            <button
              onClick={(e) => {
